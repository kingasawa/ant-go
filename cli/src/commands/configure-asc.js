...
async function configureAsc({ token: existingToken } = {}) {
  ...
  try {
    ...
    saveSpinner.fail('Không lưu được credentials');
...
async function autoFetchKey() {
  ...
  const { appleId, password } = await inquirer.prompt([
    { type: 'input',    name: 'appleId',  message: 'Apple ID (email):',                        validate: v => v.trim() ? true : 'Bắt buộc' },
    { type: 'password', name: 'password', message: 'App-Specific Password (appleid.apple.com):', validate: v => v.trim() ? true : 'Bắt buộc', mask: '•' },
  ]);
    ...
    const result = await Auth.loginAsync(
      { username: appleId.trim(), password: password.trim() },
      {
        ...
        onTwoFactorRequest: async () => {
          loginSpinner.stop();
          const { code } = await inquirer.prompt([{ type: 'input', name: 'code', message: '🔐 Nhập mã 2FA:', validate: v => /^\d{6}$/.test(v.trim()) ? true : 'Mã 6 chữ số' }]);
  ...
  try {
    ...
    if (teams.length === 1) {
      await Teams.selectTeamAsync(authCtx, { teamId: teams[0].teamId });
      console.log(chalk.green(`✔  Team: ${teams[0].name} (${teams[0].teamId})`));
    } else {
      const { selectedTeam } = await inquirer.prompt([{ type: 'list', name: 'selectedTeam', message: 'Chọn Apple Developer Team:', choices: teams.map(t => ({ name: `${t.name} (${t.teamId})`, value: t.teamId })) }]);
      await Teams.selectTeamAsync(authCtx, { teamId: selectedTeam });
    }
  } catch (err) {
    teamSpinner.fail && teamSpinner.fail('Lỗi khi lấy team');
    throw err;
  }

  // Thử detect Issuer ID từ key hiện có
  let issuerId = null;
  try {
  ...
  try {
    ...
    downloadSpinner.succeed('Private key đã download thành công');
  } catch (err) {
    downloadSpinner.fail('Không download được private key: ' + err.message);
    console.log(chalk.red(`  Key đã tạo trên App Store Connect (Key ID: ${keyId}) nhưng không download được .p8.`));
    console.log(chalk.yellow('  Download thủ công tại: https://appstoreconnect.apple.com/access/integrations/api'));
    throw err;
  }
...
async function manualInputKey() {
  ...
  const { keyId, issuerId } = await inquirer.prompt([
    { type: 'input', name: 'keyId',    message: 'Key ID (VD: 2X9R4HXF34):', validate: v => v.trim() ? true : 'Bắt buộc' },
    { type: 'input', name: 'issuerId', message: 'Issuer ID (UUID):',          validate: v => v.trim() ? true : 'Bắt buộc' },
  ]);

  const { inputMethod } = await inquirer.prompt([{
    type: 'list', name: 'inputMethod', message: 'Nội dung file .p8:',
    choices: [
  ...
  if (inputMethod === 'file') {
    const { filePath } = await inquirer.prompt([{
      type: 'input', name: 'filePath', message: 'Đường dẫn file .p8:',
      validate: (v) => { const r = path.resolve(v.trim()); return fs.existsSync(r) ? true : `Không tìm thấy file: ${r}`; },
    }]);
    privateKeyP8 = fs.readFileSync(path.resolve(filePath.trim()), 'utf8');
  } else {
    const { pasted } = await inquirer.prompt([{ type: 'editor', name: 'pasted', message: 'Paste nội dung file .p8 rồi lưu và đóng editor:' }]);
    privateKeyP8 = pasted;
  }
