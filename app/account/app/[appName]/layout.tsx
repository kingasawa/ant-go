...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      ...
      {/* User card */}
      {user && (
        <div className="relative px-4 py-4 flex-shrink-0 flex items-center gap-3" style={{ borderTop: "1px solid var(--dash-border)" }}>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "var(--dash-bg)" }}>
      ...
      {/* Desktop sidebar */}
      <aside
        className="hidden lg:flex w-60 flex-shrink-0 flex-col sticky top-0 h-screen"
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-start pb-4 pl-4 lg:pb-6 lg:pl-6">
          ...
          >
            <div className="flex items-center gap-3 px-4 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <img src={user.photoURL ?? "/avatar.png"} alt="" className="w-10 h-10 rounded-full object-cover border-2 border-purple/40 flex-shrink-0" />
            ...
            <div className="py-1">
              <button
                onClick={() => { setUserMenuOpen(false); router.push("/account/profile"); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/5 transition"
              >
              ...
              <button
                onClick={() => { setUserMenuOpen(false); router.push("/account/usage"); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/5 transition"
              >
              ...
              <button
                onClick={() => { setUserMenuOpen(false); router.push("/account/billing"); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-white/70 hover:text-white hover:bg-white/5 transition"
              >
              ...
              <button
                onClick={() => { setUserMenuOpen(false); signOut(auth).then(() => router.replace("/login")); }}
                className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-white/5 transition"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
