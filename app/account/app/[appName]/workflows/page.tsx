...
export default function WorkflowsPage() {
  ...
  return (
    <div>
          ...
          <div className="rounded-2xl overflow-hidden" style={GLASS}>
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div className="flex items-center gap-3">
                <FaGithub className="w-5 h-5 text-white/50 flex-shrink-0" />
                <div>
                  <p className="text-sm font-semibold text-white">GitHub Repository</p>
                  <p className="text-xs text-white/40 mt-0.5">
                    Kết nối repo để tự động trigger build.
                  </p>
                </div>
              </div>
              <button
                onClick={() => setShowConnectForm((v) => !v)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-accent/20 text-accent-light hover:bg-accent/30 transition flex-shrink-0"
              >
                <FaGithub className="w-4 h-4" />
                {showConnectForm ? "Huỷ" : "Connect GitHub"}
              </button>
            </div>
            ...
            {showConnectForm && (
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div className="px-5 py-3.5 flex items-start gap-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      {/* ── Workflows list (only when connected) ────────────────── */}
      {isConnected && (
        <div>
          {/* success banner */}
          {githubConnected && (
            <div className="mb-4 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-green-500/15 border border-green-500/30">
