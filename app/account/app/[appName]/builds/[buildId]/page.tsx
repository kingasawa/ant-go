  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-3 mb-5 flex-shrink-0 flex-wrap">
        ...
        <span className="text-sm font-mono text-gray-600 dark:text-gray-400 truncate max-w-[180px]">{buildId}</span>
        ...
        {build && (
          <span className={`inline-block border text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase ${STATUS_COLOR[build.status] ?? "bg-gray-100 text-gray-500 border-gray-300"}`}>
            {build.status}
          </span>
        )}

        {isActive && (
          <span className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wide text-green-500">
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {build?.createdAt?.seconds && (
            <span className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 font-mono">
              ...
              {isActive && <span className="text-green-500 animate-pulse ml-0.5">·</span>}
            </span>
          )}
        </div>
      </div>
      ...
      {buildError && (
        <div className="mb-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl px-4 py-3">
          <p className="text-sm font-semibold text-red-600 dark:text-red-400 mb-1">⚠ Không thể tải build</p>
          <p className="text-xs text-red-500 font-mono break-all">{buildError}</p>
        </div>
      )}
      ...
      {isStuck && !warnDismissed && (
        <div className="mb-4 flex items-start gap-3 bg-orange-950/40 border border-orange-700/50 rounded-xl px-4 py-3">
      ...
      {/* Tabs */}
      <div className="flex border-b border-gray-200 dark:border-gray-800 mb-4 flex-shrink-0">
      ...
      {tab === "logs" ? (
        ...
        <div className="space-y-5 overflow-y-auto">
          {buildLoading ? (
            <div className="text-center py-16 text-gray-400 animate-pulse text-sm">Đang tải...</div>
          ) : (
            <>
              <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
                {infoRows.map((r) => (
                  <div key={r.label} className="flex items-start justify-between gap-4 px-5 py-3.5">
                    <span className="text-xs text-gray-500 dark:text-gray-400 flex-shrink-0 pt-0.5 w-28">{r.label}</span>
